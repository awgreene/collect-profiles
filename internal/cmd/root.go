...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&tlsCertPath, "tls-cert-path", "", "/etc/pki/tls/certs/tls.cert", "The path to the tls cert used by the client making https requests against the pprof endpoints.")
	rootCmd.PersistentFlags().BoolVarP(&garbageCollection, "garbage-collection", "", false, "If true, configMaps generated by previous runs of the command are deleted.")
}
...
func newCmd() *cobra.Command {
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.ToLower(config.Data[suspend]) == "true" {
			...
			newestConfigMaps := []corev1.ConfigMap{}
			errs := []error{}
			if garbageCollection {
				...
				var expiredConfigMaps []corev1.ConfigMap
			...
			for _, a := range validatedArguments {
				...
				if response.StatusCode != 200 {
					klog.Errorf("%s responded with %d status code instead of 200", a.endpoint, response.StatusCode)
					continue
				}
